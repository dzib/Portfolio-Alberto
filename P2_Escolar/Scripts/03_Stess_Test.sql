...
--- ----------------------------
IF NOT EXISTS (SELECT 1 FROM sys.tables WHERE name = 'Inscripciones' AND schema_id = SCHEMA_ID('Operaciones'))
BEGIN
...
--- -- ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        IF NOT EXISTS (SELECT 1 FROM Catalogos.Cursos WHERE Nombre LIKE 'NCurso_%')
        BEGIN
            INSERT INTO Catalogos.Cursos (Nombre) SELECT 'NCurso_' + CAST(ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS VARCHAR(15))
            ...
            CROSS JOIN ( SELECT 1 UNION ALL SELECT 1 UNION ALL SELECT 1 UNION ALL SELECT 1 UNION ALL SELECT 1) b(n); -- 50 ejemplo
        END
        ...
        IF NOT EXISTS (SELECT 1 FROM Operaciones.Materias WHERE Nombre LIKE 'NMateria_%')
        BEGIN
            INSERT INTO Operaciones.Materias (Nombre)  SELECT 'NMateria_' + CAST(ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS VARCHAR(15))
    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            -- Realizamos la extracción del token EstatusAcademico para decidir cantidad, pero sin usar funciones de texto complejas en la consulta principal.
                SELECT TOP (@BatchSize) A.AlumnoID,  -- extraer EstatusAcademico token para decidir cantidad
                    CASE WHEN CHARINDEX('|',A.MetaData_ETL) = 0 THEN NULL ELSE UPPER(LTRIM(RTRIM( SUBSTRING( A.MetaData_ETL, CHARINDEX('|',A.MetaData_ETL) + 1,
            ...
            CROSS APPLY (
                SELECT n FROM ( SELECT ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS n
            ...
            WHERE Cnt.Cantidad > 0 AND NOT EXISTS ( SELECT 1
            ...
            -- Log y checkpoint.
            INSERT INTO Control.LoadLog (RunNumber, Entidad, BatchOffset, RowsAffected, Estado)
            VALUES (@CurrentRun, 'Inscripciones', @OffsetIns, @RowsIns, 'COMMIT');
        ...
        BEGIN CATCH
            IF XACT_STATE() <> 0 ROLLBACK;
            INSERT INTO Control.LoadLog (RunNumber, Entidad, BatchOffset, RowsAffected, Estado, Mensaje)
            VALUES (@CurrentRun, 'Inscripciones', @OffsetIns, 0, 'ROLLBACK', ERROR_MESSAGE());
...
--- -- ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    BEGIN TRY
        ...
        INSERT INTO Control.LoadLog (RunNumber, Entidad, BatchOffset, RowsAffected, Estado)
        VALUES (@CurrentRun, 'Calificaciones', 0, @RowsPar, 'COMMIT');
    ...
    BEGIN CATCH
        IF XACT_STATE() <> 0 ROLLBACK;
        INSERT INTO Control.LoadLog (RunNumber, Entidad, BatchOffset, RowsAffected, Estado, Mensaje)
        VALUES (@CurrentRun, 'Calificaciones', 0, 0, 'ROLLBACK', ERROR_MESSAGE());
...
--- -- ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            UPDATE I
            SET NotaFinal = P.Prom FROM Operaciones.Inscripciones I
        ...
        BEGIN CATCH
            IF XACT_STATE() <> 0 ROLLBACK;
            INSERT INTO Control.LoadLog  (RunNumber, Entidad, BatchOffset, RowsAffected, Estado, Mensaje) VALUES
                (@CurrentRun, 'NotaFinal', 0, 0, 'ROLLBACK', ERROR_MESSAGE());
